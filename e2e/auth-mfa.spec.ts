...
async function mockAuth(page: Page, enrollmentRequired: boolean) {
  ...
  await page.route("**/api/v1/auth/**", async (route) => {
    ...
    if (path.endsWith("/auth/csrf")) {
      csrfSequence += 1;
      await route.fulfill({ status: 200, headers: cors, body: JSON.stringify({ data: { csrfToken: `csrf-${csrfSequence}` }, meta: { requestId: `csrf-${csrfSequence}` } }) });
      return;
    }
    expect(request.headers()["x-csrf-token"]).toBe(`csrf-${csrfSequence}`);
  ...
  return { enrollCalls: () => enrollCalls };
}
