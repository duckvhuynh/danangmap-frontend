...
import { IconArrowRight, IconDatabase, IconProgressCheck, IconPencil } from "@tabler/icons-react";
...
export default function AdminDashboardPage() {
  ...
  return <main className="mx-auto max-w-[1200px] p-4 pb-24 sm:p-6 md:p-8">
    <header className="flex flex-wrap items-start justify-between gap-4">
      ...
      <Button asChild><Link href="/admin/layers">Quản lý lớp dữ liệu<IconArrowRight data-icon="inline-end" stroke={1.75}/></Link></Button>
    </header>
    ...
    <section className="mt-7 grid gap-4 sm:grid-cols-3" aria-label="Tình hình lớp dữ liệu">
      {layers === null && error === null ? <div className="contents" role="status" aria-label="Đang tải tổng quan"><span className="sr-only">Đang tải tổng quan...</span>{[0, 1, 2].map((item) => <Skeleton key={item} className="h-28 rounded-panel"/>)}</div> :
        metrics.map(({ label, value, icon: Icon }) => <article key={label} className="rounded-panel border bg-surface p-5"><div className="flex items-center justify-between"><h2 className="text-sm text-muted-foreground">{label}</h2><Icon aria-hidden="true" className="text-primary" stroke={1.75}/></div><p className="mt-3 text-3xl font-semibold tabular-nums">{value}</p></article>)}
    </section>
    {layers !== null && <section className="mt-7" aria-labelledby="admin-tasks-title">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Các lớp đang biên tập hoặc chờ duyệt, công bố phù hợp với vai trò của bạn.</p>
      {tasks.length === 0 ? <Empty className="mt-4 border bg-surface"><EmptyHeader><EmptyTitle>Chưa có công việc cần xử lý</EmptyTitle><EmptyDescription>Bạn có thể mở danh sách lớp để xem dữ liệu hiện có.</EmptyDescription></EmptyHeader><Button asChild variant="outline"><Link href="/admin/layers">Xem lớp dữ liệu</Link></Button></Empty> :
        <ul className="mt-4 divide-y rounded-panel border bg-surface">{tasks.slice(0, 6).map((layer) => <li key={layer.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
          ...
          <Button asChild variant="outline" size="sm"><Link href={isEditableStatus(layer.status) || !layer.revisionId ? `/admin/layers/${layer.id}` : `/admin/layers/${layer.id}/revisions/${layer.revisionId}/review`}>{isEditableStatus(layer.status) ? "Mở lớp" : "Xem để xử lý"}<IconArrowRight data-icon="inline-end" stroke={1.75}/></Link></Button>
        </li>)}</ul>}
      {tasks.length > 6 && <Button asChild variant="ghost" className="mt-3"><Link href="/admin/layers">Xem tất cả lớp dữ liệu<IconArrowRight data-icon="inline-end" stroke={1.75}/></Link></Button>}
    </section>}
  </main>;
}
