...
import { NewLayerConfigurationScreen, type LayerConfigurationCreateTransport } from "./new-layer-configuration-screen";
...
describe("new layer configuration screen", () => {
  it("loads groups only for a desktop Editor", async () => {
    ...
    expect(screen.getByRole("status", { name: "Đang tải cấu hình layer" })).toBeInTheDocument();
  ...
  it.each(["reviewer", "publisher", "system_admin"] as const)("denies %s without loading authoring data", (role) => {
    vi.mocked(useAdminSession).mockReturnValue({
  ...
  it("surfaces a durable group-loading error", async () => {
    const api = transport();
    vi.mocked(api.listGroups).mockRejectedValue(new Error("Không tải được nhóm lớp"));
    render(<NewLayerConfigurationScreen transport={api}/>);
    expect(await screen.findByRole("alert")).toHaveTextContent("Không tải được nhóm lớp");
  });
});
