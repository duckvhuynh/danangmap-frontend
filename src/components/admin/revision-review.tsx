...
import { AdminApiError, approveRevision, loadRevisionBundle, publishRevision, requestRevisionChanges, type RevisionBundle } from "@/lib/api/admin";
...
export function RevisionReview({ revisionId }: { revisionId: string }) {
  ...
  const publisherAction = principal.role === "publisher" && bundle.revision.status === "approved" && !publicationStale;
  ...
  {error !== null && <div className="space-y-2"><AdminErrorNotice error={error} onRetry={load}/>{activeRevisionId && <Button asChild variant="outline" className="w-full"><Link href={`/admin/layers/${activeRevisionId}/review`}>Mở revision đang công bố</Link></Button>}</div>} {success && <p className="rounded-control bg-emerald-50 p-3 text-sm text-success" role="status">{success}</p>}
