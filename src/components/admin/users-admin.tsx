...
function userErrorMessage(error: unknown) {
  if (error instanceof AdminApiError) {
    if (error.status === 409) return `Yêu cầu xung đột với dữ liệu tài khoản hiện có. ${error.message}${error.requestId ? ` Mã yêu cầu: ${error.requestId}.` : ""}`;
    if (error.status === 412) return `Dữ liệu đã thay đổi trên máy chủ. Tải lại trước khi thử tiếp.${error.requestId ? ` Mã yêu cầu: ${error.requestId}.` : ""}`;
    if (error.status === 422) return `Dữ liệu tài khoản chưa hợp lệ. ${error.message}${error.requestId ? ` Mã yêu cầu: ${error.requestId}.` : ""}`;
  }
...
function AccountDialog({ mode, open, onOpenChange, onCreated }: { mode: AccountCreationMode; open: boolean; onOpenChange(open: boolean): void; onCreated(message: string): void }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent aria-describedby="account-dialog-description"><DialogHeader><DialogTitle>{isManual ? "Tạo tài khoản thủ công" : "Gửi lời mời tài khoản"}</DialogTitle><DialogDescription id="account-dialog-description">{isManual ? "Cấp mật khẩu tạm thời. Người dùng phải đổi mật khẩu theo chính sách khi đăng nhập." : "Người nhận hoàn tất thiết lập tài khoản qua lời mời có thời hạn."}</DialogDescription></DialogHeader><form className="grid gap-5" onSubmit={submit} noValidate>{requestError ? <Alert variant="destructive"><IconAlertCircle stroke={1.75} /><AlertTitle>Không thể hoàn tất yêu cầu</AlertTitle><AlertDescription>{userErrorMessage(requestError)}</AlertDescription></Alert> : null}<FieldGroup className="gap-4">
    <Field data-invalid={Boolean(errors.displayName)}><FieldLabel htmlFor="account-displayName">Tên hiển thị</FieldLabel><Input id="account-displayName" value={values.displayName} onChange={(event) => change("displayName", event.target.value)} autoComplete="name" aria-invalid={Boolean(errors.displayName)} disabled={submitting} /><FieldError>{errors.displayName}</FieldError></Field>
    <div className="grid gap-4 sm:grid-cols-2"><Field data-invalid={Boolean(errors.username)}><FieldLabel htmlFor="account-username">Tên đăng nhập</FieldLabel><Input id="account-username" value={values.username} onChange={(event) => change("username", event.target.value.toLocaleLowerCase("vi"))} autoComplete="username" spellCheck={false} aria-invalid={Boolean(errors.username)} disabled={submitting} /><FieldDescription>Ví dụ: nguyenvana</FieldDescription><FieldError>{errors.username}</FieldError></Field><Field data-invalid={Boolean(errors.email)}><FieldLabel htmlFor="account-email">Email công vụ</FieldLabel><Input id="account-email" type="email" value={values.email} onChange={(event) => change("email", event.target.value)} autoComplete="email" spellCheck={false} aria-invalid={Boolean(errors.email)} disabled={submitting} /><FieldError>{errors.email}</FieldError></Field></div>
...
export function UsersAdmin() {
  ...
  if (principal.role !== "system_admin") return <main className="mx-auto grid min-h-[70dvh] max-w-2xl place-items-center p-4 pb-24 sm:p-6 md:p-8"><Alert variant="destructive"><IconShieldLock stroke={1.75} /><AlertTitle>Không có quyền truy cập</AlertTitle><AlertDescription>Trang người dùng chỉ dành cho Quản trị hệ thống. Danh sách tài khoản chưa được tải.</AlertDescription></Alert></main>;
  ...
  return <main className="mx-auto max-w-[1200px] p-4 pb-24 sm:p-6 md:p-8">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-muted-foreground">Quản trị truy cập</p><h1 className="mt-1 text-2xl font-semibold">Người dùng nội bộ</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Quản lý tài khoản, vai trò, phiên, MFA và mail bảo mật từ một directory có kiểm soát phiên bản.</p></div>{canMutate ? <div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/admin/users/import"><IconFileImport stroke={1.75} />Import</Link></Button><Button type="button" variant="outline" onClick={() => { setSuccess(""); setDialogMode("invite"); }}><IconMailPlus stroke={1.75} />Mời</Button><Button type="button" onClick={() => { setSuccess(""); setDialogMode("manual"); }}><IconPlus stroke={1.75} />Tạo tài khoản</Button></div> : null}</header>
    {!canMutate ? <Alert className="mt-5 border-primary/20 bg-accent-subtle"><IconDeviceDesktop stroke={1.75} /><AlertTitle>Chế độ chỉ xem</AlertTitle><AlertDescription>Thiết bị này có thể xem directory và chi tiết bảo mật. Mọi mutation chỉ xuất hiện trên desktop có bàn phím và thiết bị trỏ chính xác.</AlertDescription></Alert> : null}
    {success ? <div className="mt-5 flex items-start gap-3 rounded-control border border-success/20 bg-green-50 p-4 text-sm text-success" role="status" aria-live="polite"><IconCheck className="mt-0.5 shrink-0" stroke={1.75} /><span>{success}</span></div> : null}

    <div className="mt-7 flex w-fit rounded-control border bg-surface p-1" role="tablist" aria-label="Directory tài khoản"><button type="button" role="tab" aria-selected={section === "users"} className={`flex min-h-10 items-center gap-2 rounded-control px-4 text-sm font-medium ${section === "users" ? "bg-accent-subtle text-primary" : "text-muted-foreground hover:bg-surface-subtle"}`} onClick={() => setSection("users")}><IconUsers size={18} stroke={1.75} />Tài khoản</button><button type="button" role="tab" aria-selected={section === "invites"} className={`flex min-h-10 items-center gap-2 rounded-control px-4 text-sm font-medium ${section === "invites" ? "bg-accent-subtle text-primary" : "text-muted-foreground hover:bg-surface-subtle"}`} onClick={() => setSection("invites")}><IconMailForward size={18} stroke={1.75} />Lời mời</button></div>

    {section === "users" ? <section className="mt-4 overflow-hidden rounded-panel border bg-surface" aria-labelledby="users-list-title">
      <div className="grid gap-4 border-b p-4 lg:grid-cols-[1fr_12rem_12rem]"><div className="relative"><IconSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={19} stroke={1.75} /><label className="sr-only" htmlFor="users-search">Tìm tài khoản</label><Input id="users-search" className="pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm tên, email hoặc tài khoản..." /></div><Select value={role} onValueChange={(value: AdminUser["role"] | "all") => setRole(value)}><SelectTrigger aria-label="Lọc vai trò tài khoản"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="all">Tất cả vai trò</SelectItem>{adminUserRoles.map((item) => <SelectItem key={item} value={item}>{adminUserRoleLabels[item]}</SelectItem>)}</SelectGroup></SelectContent></Select><Select value={status} onValueChange={(value: AdminUser["status"] | "all") => setStatus(value)}><SelectTrigger aria-label="Lọc trạng thái tài khoản"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="all">Tất cả trạng thái</SelectItem><SelectItem value="active">Đang hoạt động</SelectItem><SelectItem value="inactive">Chưa hoạt động</SelectItem><SelectItem value="disabled">Đã vô hiệu hóa</SelectItem><SelectItem value="invited">Đã mời</SelectItem></SelectGroup></SelectContent></Select></div>
      <div className="flex items-center justify-between gap-3 border-b px-4 py-3"><div><h2 id="users-list-title" className="font-medium">Danh sách tài khoản</h2><p className="mt-1 text-xs text-muted-foreground">{page ? `${page.data.length} tài khoản đã tải${page.meta.hasMore ? " · còn dữ liệu" : ""}` : "Directory phân trang từ hệ thống tài khoản"}</p></div>{loading && page ? <span className="flex items-center gap-2 text-xs text-muted-foreground"><Spinner />Đang làm mới</span> : null}</div>
      {loadError && !page ? <div className="p-4"><Alert variant="destructive"><IconAlertCircle stroke={1.75} /><AlertTitle>Không tải được danh sách</AlertTitle><AlertDescription><p>{userErrorMessage(loadError)}</p><Button type="button" variant="outline" size="sm" className="mt-3" onClick={() => setReloadVersion((value) => value + 1)}><IconRefresh stroke={1.75} />Thử lại</Button></AlertDescription></Alert></div> : null}
      {loading && !page ? <UsersLoading /> : page ? <UsersList users={page.data} onSelect={setSelectedUserId} /> : null}
      ...
      {loadError && page ? <div className="border-t p-4"><Alert variant="destructive"><IconAlertCircle stroke={1.75} /><AlertTitle>Danh sách hiện tại chưa được làm mới</AlertTitle><AlertDescription>{userErrorMessage(loadError)}</AlertDescription></Alert></div> : null}
