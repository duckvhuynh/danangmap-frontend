...
function UsersList({ users, onSelect, mfaPolicyEnabled }: { users: AdminUser[]; onSelect(userId: string): void; mfaPolicyEnabled: boolean }) {
  if (!users.length) return <Empty className="min-h-72 border-0"><EmptyHeader><EmptyMedia variant="icon"><IconUsers stroke={1.75} /></EmptyMedia><EmptyTitle>Chưa có tài khoản phù hợp</EmptyTitle><EmptyDescription>Thử từ khóa khác hoặc chọn lại bộ lọc.</EmptyDescription></EmptyHeader></Empty>;
  return <>
...
function AccountDialog({ mode, open, onOpenChange, onCreated }: { mode: AccountCreationMode; open: boolean; onOpenChange(open: boolean): void; onCreated(message: string): void }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent aria-describedby="account-dialog-description"><DialogHeader><DialogTitle>{isManual ? "Tạo tài khoản" : "Gửi lời mời tài khoản"}</DialogTitle><DialogDescription id="account-dialog-description">{isManual ? "Người dùng sẽ đăng nhập bằng mật khẩu tạm thời và đổi mật khẩu trong lần đầu." : "Người nhận hoàn tất thiết lập tài khoản qua lời mời có thời hạn."}</DialogDescription></DialogHeader><form className="grid gap-5" onSubmit={submit} noValidate>{requestError ? <Alert variant="destructive"><IconAlertCircle stroke={1.75} /><AlertTitle>Không thể hoàn tất yêu cầu</AlertTitle><AlertDescription>{userErrorMessage(requestError)}</AlertDescription></Alert> : null}<FieldGroup className="gap-4">
...
export function UsersAdmin() {
  ...
  if (principal.role !== "system_admin") return <main className="mx-auto grid min-h-[70dvh] max-w-2xl place-items-center p-4 pb-24 sm:p-6 md:p-8"><Alert variant="destructive"><IconShieldLock stroke={1.75} /><AlertTitle>Không có quyền truy cập</AlertTitle><AlertDescription>Chỉ quản trị hệ thống được quản lý tài khoản. Hãy liên hệ người phụ trách nếu bạn cần được cấp quyền.</AlertDescription></Alert></main>;
  ...
  return <main className="mx-auto max-w-[1200px] p-4 pb-24 sm:p-6 md:p-8">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-muted-foreground">Quản trị truy cập</p><h1 className="mt-1 text-2xl font-semibold">Người dùng nội bộ</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Tạo tài khoản, phân quyền và hỗ trợ người dùng đăng nhập.</p></div>{canMutate ? <div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/admin/users/import"><IconFileImport stroke={1.75} />Nhập từ tệp</Link></Button><Button type="button" variant="outline" onClick={() => { setSuccess(""); setDialogMode("invite"); }}><IconMailPlus stroke={1.75} />Gửi lời mời</Button><Button type="button" onClick={() => { setSuccess(""); setDialogMode("manual"); }}><IconPlus stroke={1.75} />Tạo tài khoản</Button></div> : null}</header>
    {!canMutate ? <Alert className="mt-5 border-primary/20 bg-accent-subtle"><IconDeviceDesktop stroke={1.75} /><AlertTitle>Chế độ chỉ xem</AlertTitle><AlertDescription>Bạn có thể xem tài khoản và thông tin bảo mật. Để chỉnh sửa, hãy dùng máy tính có bàn phím và chuột.</AlertDescription></Alert> : null}
