...
export function ReviewMapPreview({ revision, features }: { revision: AdminRevision; features: AdminFeature[] }) {
  ...
  return <div className="relative h-[21rem] md:h-auto md:aspect-[4/3]" role="region" aria-label="Bản đồ xem trước phiên bản, chỉ đọc"><PublicMapCanvas features={features.map((feature) => publicFeature(feature, revision.layerId, revision.title))} layers={[layer]} hiddenLayerIds={new Set()} layerColors={{ [revision.layerId]: "#1A73E8" }} basemap="light" command={{ id: 0, type: "reset" }} onFeatureSelect={() => undefined} onError={() => undefined}/><span className="absolute bottom-3 left-3 rounded-control bg-surface px-2 py-1 text-xs text-muted-foreground map-control-shadow">Bản xem phiên bản · chỉ đọc</span></div>;
}
