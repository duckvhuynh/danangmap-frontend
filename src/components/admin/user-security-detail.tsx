...
import { adminUserRoleLabels, adminUserRoles, adminUserStatusLabels } from "@/lib/users/user-admin-state";
...
const actionCopy: Record<SecurityAction["kind"], { title: string; description: string; button: string; destructive?: boolean }> = {
  "revoke-session": { title: "Thu hồi phiên đăng nhập", description: "Phiên đã chọn sẽ bị vô hiệu ngay và thiết bị đó phải đăng nhập lại.", button: "Thu hồi phiên", destructive: true },
  "revoke-all": { title: "Thu hồi mọi phiên", description: "Mọi phiên đăng nhập và phiên xác thực tạm thời của tài khoản sẽ bị vô hiệu.", button: "Thu hồi tất cả", destructive: true },
  "reset-mfa": { title: "Đặt lại MFA", description: "MFA và toàn bộ phiên sẽ bị thu hồi. Người dùng phải đăng ký MFA lại ở lần đăng nhập tiếp theo.", button: "Đặt lại MFA", destructive: true },
  "password-reset": { title: "Gửi yêu cầu đặt lại mật khẩu", description: "Hệ thống chỉ xếp mail vào hàng đợi; mã đặt lại không bao giờ hiển thị tại đây.", button: "Gửi mail đặt lại" },
};
...
function detailErrorMessage(error: unknown) {
  if (error instanceof AdminApiError) {
    const messages: Record<number, string> = {
      ...
      428: "Thiếu phiên bản dữ liệu. Hãy tải lại chi tiết tài khoản.",
      429: "Có quá nhiều thao tác bảo mật. Hãy chờ rồi thử lại.",
    };
    const prefix = messages[error.status];
    if (prefix) return `${prefix}${error.requestId ? ` Mã yêu cầu: ${error.requestId}.` : ""}`;
  }
      ...
      <DialogContent>
        ...
        <form className="grid gap-5" onSubmit={submit}>
          ...
          <Field data-invalid={reason.length > 0 && reason.trim().length < 8}>
            ...
            <FieldDescription>Ít nhất 8 ký tự. Nội dung này được lưu trong audit, không nhập thông tin bí mật.</FieldDescription>
...
export function UserSecurityDetail({
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setResource(next);
      hydrateForm(next.data);
      onChanged("Đã cập nhật tài khoản và làm mới phiên bản dữ liệu.");
    } catch (caught) {
      setSaveError(caught);
    } finally {
      setSaving(false);
  ...
  async function submitSecurityAction(action: SecurityAction, actionReason: string, operationKey: string) {
    ...
    await load();
    onChanged(action.kind === "password-reset" ? "Đã xếp mail đặt lại mật khẩu vào hàng đợi." : "Đã hoàn tất thao tác bảo mật và làm mới chi tiết tài khoản.");
  }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl p-0">
          <DialogHeader className="border-b px-5 py-5 sm:px-6">
            ...
            <DialogDescription>Hồ sơ, trạng thái truy cập và các sự kiện bảo mật gần đây. Không có secret hoặc token trong response.</DialogDescription>
          </DialogHeader>
          <div className="grid max-h-[calc(100dvh-11rem)] gap-6 overflow-y-auto px-5 pb-6 sm:px-6">
            ...
            {user ? <>
              ...
              <section className="rounded-panel border bg-surface p-4" aria-labelledby="account-fields-title">
                <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-control bg-accent-subtle text-primary"><IconUserEdit stroke={1.75} /></span><div><h3 id="account-fields-title" className="font-medium">Hồ sơ và quyền truy cập</h3><p className="mt-1 text-xs text-muted-foreground">Đổi vai trò hoặc trạng thái sẽ thu hồi phiên theo contract.</p></div></div>
                {canMutate ? <form className="mt-5 grid gap-4" onSubmit={save}>
                  ...
                  {hasRoleOrStatusChange ? <Field data-invalid={reason.length > 0 && reason.trim().length < 8}><FieldLabel htmlFor="admin-user-change-reason">Lý do thay đổi quyền</FieldLabel><Input id="admin-user-change-reason" value={reason} onChange={(event) => { setReason(event.target.value); editChanged(); }} disabled={saving} maxLength={500} /><FieldDescription>Ít nhất 8 ký tự, được lưu trong audit.</FieldDescription><FieldError>{reason.length > 0 && reason.trim().length < 8 ? "Lý do cần ít nhất 8 ký tự." : undefined}</FieldError></Field> : null}
              ...
              <section className="rounded-panel border bg-surface p-4" aria-labelledby="mail-history-title"><div className="flex items-center gap-3"><IconClock className="text-primary" stroke={1.75} /><div><h3 id="mail-history-title" className="font-medium">Lời mời và mail đặt lại gần đây</h3><p className="mt-1 text-xs text-muted-foreground">Chỉ hiển thị trạng thái giao nhận an toàn.</p></div></div><div className="mt-4 grid gap-2 text-sm">{[...user.invites.map((item) => ({ id: item.id, label: `Lời mời · ${item.status}`, at: item.createdAt, mail: item.mailStatus })), ...user.passwordResets.map((item) => ({ id: item.id, label: `Đặt lại mật khẩu · ${item.status}`, at: item.createdAt, mail: item.mailStatus }))].map((item) => <div key={item.id} className="flex flex-wrap justify-between gap-2 rounded-control bg-surface-subtle px-3 py-2"><span>{item.label}</span><span className="text-xs text-muted-foreground">Mail: {item.mail ?? "chưa có"} · {formatDate(item.at)}</span></div>)}{!user.invites.length && !user.passwordResets.length ? <p className="text-muted-foreground">Chưa có mail bảo mật gần đây.</p> : null}</div><Button asChild className="mt-4" variant="outline" size="sm"><Link href="/admin/audit"><IconShieldLock stroke={1.75} />Mở audit toàn hệ thống</Link></Button></section>
            </> : null}
          </div>
        </DialogContent>
      </Dialog>
