      ...
      <DialogContent>
        ...
        <form className="grid gap-5" onSubmit={submit}>
          ...
          <Field data-invalid={reason.length > 0 && reason.trim().length < 8}>
            ...
            <Input id="security-action-reason" value={reason} placeholder="Ví dụ: Khóa tài khoản theo yêu cầu của đơn vị" onChange={(event) => { setReason(event.target.value); setError(null); operationKey.current = null; }} maxLength={500} autoFocus disabled={pending} />
            ...
            {user ? <>
              ...
              <section className="rounded-panel border bg-surface p-4" aria-labelledby="account-fields-title">
                ...
                {canMutate ? <form className="mt-5 grid gap-4" onSubmit={save}>
                  ...
                  <Field data-invalid={displayName.trim().length < 2}><FieldLabel htmlFor="admin-user-display-name">Tên hiển thị</FieldLabel><Input id="admin-user-display-name" value={displayName} placeholder="Ví dụ: Nguyễn Văn An" onChange={(event) => { setDisplayName(event.target.value); editChanged(); }} disabled={saving} maxLength={200} /><FieldError>{displayName.trim().length < 2 ? "Tên hiển thị cần ít nhất 2 ký tự." : undefined}</FieldError></Field>
                  ...
                  {hasRoleOrStatusChange ? <Field data-invalid={reason.length > 0 && reason.trim().length < 8}><FieldLabel htmlFor="admin-user-change-reason">Lý do thay đổi quyền</FieldLabel><Input id="admin-user-change-reason" value={reason} placeholder="Ví dụ: Phân công nhiệm vụ kiểm duyệt dữ liệu" onChange={(event) => { setReason(event.target.value); editChanged(); }} disabled={saving} maxLength={500} /><FieldDescription>Ít nhất 8 ký tự để ghi lại trong lịch sử hoạt động.</FieldDescription><FieldError>{reason.length > 0 && reason.trim().length < 8 ? "Lý do cần ít nhất 8 ký tự." : undefined}</FieldError></Field> : null}
