...
describe("system audit screen", () => {
  ...
  it("loads role-scoped audit and preserves the opaque pagination cursor", async () => {
    ...
    expect(screen.getByRole("main", { name: "Audit toàn hệ thống" })).toHaveAttribute("aria-busy", "false");
    expect(screen.getByRole("status")).toHaveTextContent("Đã tải 1 sự kiện kiểm toán.");
    const summary = screen.getByLabelText(/Metadata đã lọc cho user\.invited/u);
    fireEvent.click(summary);
    const metadataContent = screen.getByLabelText(/Nội dung metadata đã lọc cho user\.invited/u);
    metadataContent.focus();
    expect(metadataContent).toHaveFocus();
    const loadMore = screen.getByRole("button", { name: "Tải thêm sự kiện" });
    expect(loadMore).toHaveAttribute("aria-controls", screen.getByRole("list", { name: "Sự kiện kiểm toán" }).id);
    loadMore.focus();
    ...
    expect(screen.getByRole("list", { name: "Sự kiện kiểm toán" })).toHaveFocus();
    expect(screen.getByRole("status")).toHaveTextContent("Đã tải 2 sự kiện kiểm toán.");
  });
});
