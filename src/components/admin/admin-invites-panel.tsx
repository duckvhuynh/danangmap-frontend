...
import { IconAlertCircle, IconClock, IconDeviceDesktop, IconMailForward, IconRefresh, IconSearch, IconX } from "@tabler/icons-react";
  ...
  return <>
    <section className="overflow-hidden rounded-panel border bg-surface" aria-labelledby="invite-list-title">
      <div className="grid gap-4 border-b p-4 lg:grid-cols-[1fr_12rem_12rem]">
        <div className="relative"><IconSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={19} stroke={1.75} /><label className="sr-only" htmlFor="invite-search">Tìm lời mời</label><Input id="invite-search" className="pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm email, tên hoặc tài khoản..." /></div>
