...
export function LoginForm() {
  ...
  return <form className="mt-8 flex flex-col gap-5" onSubmit={async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); setPending(true); setError(null); try { const result = await login(String(form.get("login") ?? ""), String(form.get("password") ?? "")); if (result.status === "authenticated") { router.replace(result.principal.mustChangePassword ? "/login/password-change" : "/admin"); } else { router.push(result.mfaEnrollmentRequired ? "/login/mfa?enrollment=required" : "/login/mfa"); } } catch (caught) { setError(authErrorMessage(caught, "login")); } finally { setPending(false); } }}>
    ...
    {error && <p className="flex items-start gap-2 rounded-control bg-red-50 p-3 text-sm text-destructive" role="alert"><IconAlertCircle className="mt-0.5 shrink-0" size={18} strokeWidth={1.75}/>{error}</p>}
