...
async function reachPasswordStep() {
  vi.mocked(inspectInvite).mockResolvedValue(inspection);
  pasteToken();
...
function fillPasswords() {
  ...
  fireEvent.change(screen.getByLabelText("Nhập lại mật khẩu"), { target: { value: password } });
}
...
describe("public invite privacy and one-time actions", () => {
  it("inspects exactly once under StrictMode and double activation without persistence or logging", async () => {
    ...
    render(<StrictMode><InviteAcceptForm /></StrictMode>);
    pasteToken();
  ...
  it("uses password-manager-friendly fields, accepts once and clears sensitive rendered state before MFA navigation", async () => {
    ...
    fillPasswords();
    const submit = screen.getByRole("button", { name: "Tạo mật khẩu và tiếp tục" });
    fireEvent.click(submit);
    fireEvent.click(submit);
  ...
  it("never retries an ambiguous accept and removes token and password inputs", async () => {
    ...
    await reachPasswordStep();
    fillPasswords();
    fireEvent.click(screen.getByRole("button", { name: "Tạo mật khẩu và tiếp tục" }));

    expect(await screen.findByText("Trạng thái kích hoạt chưa xác định")).toBeInTheDocument();
    ...
    expect(document.body).not.toHaveTextContent(token);
    expect(vi.mocked(acceptInvite)).toHaveBeenCalledTimes(1);
    expect(screen.queryByRole("button", { name: "Tạo mật khẩu và tiếp tục" })).not.toBeInTheDocument();
  });
});

describe("public invite errors and accessibility", () => {
  it.each([
    ["invalid", new InviteApiError(400, "INVITE_INVALID_OR_EXPIRED", "invalid")],
    ["expired", new InviteApiError(400, "INVITE_INVALID_OR_EXPIRED", "expired")],
    ["used", new InviteApiError(400, "INVITE_INVALID_OR_EXPIRED", "used")],
    ["revoked", new InviteApiError(400, "INVITE_INVALID_OR_EXPIRED", "revoked")],
  ])("renders the same focused public message for a %s token", async (_name, failure) => {
  ...
  it.each([
  ...
  ])("surfaces an explicit %i accept error and keeps a keyboard retry path", async (status, code, expected) => {
    vi.mocked(acceptInvite).mockRejectedValue(
      new InviteApiError(status, code, "backend detail", undefined, status === 429 ? 30 : undefined),
    );
    render(<InviteAcceptForm />);
    await reachPasswordStep();
    fillPasswords();
    ...
    const retry = screen.getByRole("button", { name: "Tạo mật khẩu và tiếp tục" });
