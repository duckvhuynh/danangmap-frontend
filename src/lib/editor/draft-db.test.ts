...
import { draftDb, draftKey, shouldAutosaveDraft } from "./draft-db";
...
describe("editor recovery store", () => {
  ...
  it("persists geometry and form state by layer without credentials or file binaries", async () => {
    ...
    await draftDb.drafts.put({ id, principalId: "admin-a", layerId: "wards", draftRevision: 20, baseRevision: 19, updatedAt: "2026-08-21T08:00:00.000Z", title: "Ranh giới", description: "Bản nháp", features: [{ type: "Feature", geometry: { type: "Point", coordinates: [108.22, 16.06] } }] });
  ...
  it("keeps a recovered snapshot untouched until resume or discard is chosen", async () => {
    const id = draftKey("admin-a", "wards", 20);
    const recovered = { id, principalId: "admin-a", layerId: "wards", draftRevision: 20, baseRevision: 19, updatedAt: "2026-08-21T08:00:00.000Z", title: "Recovered title", description: "Recovered", features: [{ id: "saved-feature" }] };
