...
describe("session authentication transport", () => {
  ...
  it("acquires public CSRF then logs in with typed header and credentialed cookies", async () => {
    ...
    const fetcher: typeof fetch = async (input, init) => {
      ...
      if (request.url.endsWith("/auth/csrf"))
        return new Response(envelope({ csrfToken: "csrf-public" }), {
          status: 200,
          headers,
        });
      return new Response(
        envelope({
          ...
          challengeExpiresAt: "2026-08-21T01:00:00.000Z",
        }),
        { status: 200, headers },
      );
    };
    ...
    await expect(
      login(
        "editor@example.gov.vn",
        "very-secure-password",
        createDanangMapClient(fetcher),
      ),
  ...
  it("accepts a direct authenticated session when the server MFA policy is disabled", async () => {
    vi.stubEnv("NEXT_PUBLIC_DANANGMAP_DEMO_MODE", "false");
    let calls = 0;
    const client = createDanangMapClient(async () => {
      calls += 1;
      return calls === 1
        ? new Response(envelope({ csrfToken: "csrf-public" }), {
            status: 200,
            headers,
          })
        : new Response(
            envelope({
              ...
              mfaEnrollmentRequired: false,
              principal: { ...principal, mfaEnabled: false },
            }),
            { status: 200, headers },
          );
    });
    ...
    await expect(
      login("editor", "very-secure-password", client),
    ).resolves.toEqual({
      status: "authenticated",
      mfaEnrollmentRequired: false,
      principal: { ...principal, mfaEnabled: false },
  ...
  it("reuses the stable preauth CSRF token before verify, enrollment start and confirmation", async () => {
    ...
    const recoveryCodes = Array.from(
      { length: 10 },
      (_, index) => `ABCD-EF01-2345-6789-${String(index + 1).padStart(4, "0")}`,
    );
    const fetcher: typeof fetch = async (input, init) => {
      const request = new Request(input, init);
      requests.push(request);
      ...
      if (path.endsWith("/auth/csrf"))
        return new Response(envelope({ csrfToken: stablePreauthCsrf }), {
          status: 200,
          headers: { ...headers, "cache-control": "private, no-store" },
        });
      if (path.endsWith("/mfa/verify"))
        return new Response(envelope(principal), { status: 200, headers });
      if (path.endsWith("/mfa/enroll/confirm"))
        return new Response(envelope({ principal, recoveryCodes }), {
          status: 200,
          headers,
        });
      return new Response(
        envelope({
          status: "pending",
          enrollmentUri:
            "otpauth://totp/DanangMap%3Aeditor?secret=JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP&issuer=DanangMap",
        }),
        { status: 200, headers },
      );
    };
    ...
    expect(
      mutations.map((request) => request.headers.get("x-csrf-token")),
    ).toEqual(Array(4).fill(stablePreauthCsrf));
    expect(
      requests.filter((request) => request.url.endsWith("/auth/csrf")),
    ).toHaveLength(4);
    expect(
      mutations.every((request) => request.credentials === "include"),
    ).toBe(true);
    expect(await mutations[0].clone().json()).toEqual({
      method: "totp",
      code: "123456",
    });
    expect(await mutations[1].clone().json()).toEqual({
      method: "recovery_code",
      code: "ABCD-EF01-2345-6789-0001",
    });
  ...
  it("marks an enrollment POST network failure as ambiguous and does not hide 429 retry hints", async () => {
    ...
    await expect(startMfaEnrollment(ambiguousClient)).rejects.toMatchObject({
    ...
    const limitedClient = createDanangMapClient(async (input) => {
      if (String(input).endsWith("/auth/csrf"))
        return new Response(envelope({ csrfToken: "csrf-2" }), {
          status: 200,
          headers,
        });
      return new Response(
    ...
    const error = await verifyMfa("totp", "000000", limitedClient).catch(
      (caught) => caught,
    );
  ...
  it("validates the one-time enrollment URI and exactly ten recovery codes at the trust boundary", async () => {
    expect(
      ...
      accountLabel: "DanangMap:editor",
    });
    expect(() =>
      parseEnrollmentUri(
        "otpauth://totp/%E0%A4%A?secret=SHORT&issuer=DanangMap",
      ),
    ).toThrow(AuthApiError);

    vi.stubEnv("NEXT_PUBLIC_DANANGMAP_DEMO_MODE", "false");
    const client = createDanangMapClient(async (input) =>
      String(input).endsWith("/auth/csrf")
        ? new Response(envelope({ csrfToken: "csrf" }), {
            status: 200,
            headers,
          })
        : new Response(envelope({ principal, recoveryCodes: ["only-one"] }), {
            status: 200,
            headers,
          }),
    );
    await expect(confirmMfaEnrollment("123456", client)).rejects.toMatchObject({
