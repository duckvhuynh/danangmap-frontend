...
Publication UX follows the durable async contract. A publish `202` only means the server accepted a `queued/queued` job; any terminal or already-building 202 body is rejected as a contract error and is never presented as successful publication. The review screen recovers the authoritative job after reload and tracks its detail with ETag-aware polling, bounded `Retry-After`, and no client-side stop before `succeeded` or `failed`. The history screen reads the server job list on mount and manual refresh; it does not poll that list. Network loss or a rejected non-monotonic representation is shown as a tracking interruption while the last confirmed job remains visible. Production async activation remains fail-closed in the backend until the exact Docker-stack acceptance gate is approved.
